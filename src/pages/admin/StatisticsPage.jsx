...
// --- START OF FILE StatisticsPage.jsx (ĐÃ SỬA) ---
...
const StatisticsPage = () => {
    ...
    //         year: 'numeric',
    //         hour12: false,
    //         // YÊU CẦU JAVASCRIPT CHUYỂN ĐỔI SANG MÚI GIỜ VIỆT NAM
    //         timeZone: 'Asia/Ho_Chi_Minh'
    //     };
    ...
    const formatDateTime = (dateString) => {
        ...
        try {
            ...
            // Các hàm này sẽ trả về giá trị chính xác theo múi giờ mà server đã gửi (+07:00)
            ...
            // Ghép lại thành chuỗi theo định dạng mong muốn
                                ...
                                {currentTableData.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={itemId} className={isDeleting ? 'deleting' : ''}>
                                            ...
                                            <td>{item.user_company || '-'}</td>
                                            <td>{item.user_position || '-'}</td>
                                            {/* Dòng này bây giờ sẽ hiển thị đúng */}
                                            <td>{formatDateTime(item.check_in_time)}</td>
                                            <td>
                                                <button onClick={() => handleDeleteRecord(item)} className="delete-btn" disabled={isDeleting}>
                                                    {isDeleting ? 'Đang xóa...' : 'Xóa'}
